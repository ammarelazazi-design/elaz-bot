...
// ============================================================
// 🧠 AI REPLY — محدودة بموضوع الوكالة فقط
// ============================================================
...
قواعد صارمة:
1. لا تتكلم في أي موضوع خارج نطاق الوكالة وخدماتها إطلاقاً.
2. إذا سألك العميل عن أي شيء آخر قل بلطف: "أنا هنا بس عشان أساعدك في خدمات ELAZ 😊 تحب تعرف أكتر عن خدماتنا؟"
3. خاطب الذكر بـ يا فندم أو حضرتك أو باسمه.
4. خاطب الأنثى بـ يا هانم أو حضرتك أو باسمها.
5. الردود قصيرة ومباشرة — جملتان أو ثلاث كحد أقصى.
6. لا تعِد بأسعار محددة — قل "الأسعار بتتحدد بعد الاستشارة".`;

async function getAIReply(userText, gender, clientName) {
    try {
        const genderNote = gender === 'female'
            ? `العميلة اسمها ${clientName || 'هانم'}، خاطبيها بـ يا هانم.`
            : `العميل اسمه ${clientName || 'فندم'}، خاطبه بـ يا فندم.`;
...
// ============================================================
// 🖼️ كتالوج الأعمال — ضع روابط صورك هنا
// ============================================================
const PORTFOLIO = {
    SRV_DESIGN: [
...
        { type: "postback", title: "📅 حجز استشارة",   payload: "BOOK_CONSULT" },
...
    // إرسال صور الكتالوج
    for (const url of (PORTFOLIO[service] || [])) {
        await sleep(300);
        await sendImage(sid, url);
    }
...
    const db = loadDB();
    const appointment = { sid, name: client.name, data: text, time: new Date().toISOString() };
